...
class TrackingDataset(Dataset):
    ...
    def len(self) -> int:
        if 'dataset' in self.__dict__.keys():
            return 1
        else:
...
class TrackingDataModule(LightningDataModule):
    ...
    def __setitem__(self, __name: str, __value: Dataset) -> None:
        # __name should be either `train` or `val`
        if not self.__dict__['_datasets']:
            logger.error(
                "Datasets have not been loaded yet. Make sure to call the setup method."
            )

        self.__dict__['_datasets'][__name] = __value
        # return super().__setattr__(self, '_datasets', )
